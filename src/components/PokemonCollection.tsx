...
export const PokemonCollection = ({ 
  ...
  const getRandomPokemon = (): Pokemon => {
    ...
    // Simple mock Pokémon data (in a real app, this would use PokéAPI)
    const pokemonList = {
      common: [
    ...
    const availablePokemon = pokemonList[rarity];
        ...
        <CardContent className="space-y-4">
          ...
          {lastAcquired && (
            <div className="mt-4 p-3 bg-white/10 rounded-lg text-center">
              <div className="text-2xl mb-2">{lastAcquired.sprite}</div>
          ...
          {pokemon.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {pokemon.slice(-6).map((poke, index) => (
                ...
                >
                  <div className="text-2xl mb-2">{poke.sprite}</div>
