...
export const PokeBallSelector = ({
  userLevel,
  currentXP,
  onPokemonAcquired
}: PokeBallSelectorProps) => {
  ...
  const getRandomPokemon = async (ballType: BallType): Promise<Pokemon> => {
    ...
    try {
      ...
      const response = await fetch(`https://api.pokemontcg.io/v2/cards?q=${rarityQuery}&pageSize=250`, {
        headers: {
  ...
  return <Card className="bg-gradient-to-r from-indigo-500 to-purple-600 dark:from-indigo-700 dark:to-purple-800 text-white border-0 shadow-xl">
      ...
      <CardContent className="space-y-4 bg-[#000a05]/0">
        {isOpening && selectedBall ? <div className="text-center space-y-4 py-8">
            ...
            <div className="space-y-3">
              ...
              <div className="w-full bg-white/20 rounded-full h-3 overflow-hidden">
                <div className="h-full bg-gradient-to-r from-yellow-400 to-yellow-200 rounded-full animate-pulse shadow-inner" style={{
              width: '100%'
            }} />
              </div>
            </div>
          ...
          return <div key={ballType} className={`p-5 rounded-xl border-2 transition-all duration-300 transform hover:scale-105 ${canUse ? 'border-white/50 bg-white/20 hover:bg-white/30 cursor-pointer shadow-lg hover:shadow-xl dark:border-white/60 dark:bg-white/15 dark:hover:bg-white/25' : 'border-white/30 bg-white/10 opacity-60 dark:border-white/40 dark:bg-white/8'}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <Button onClick={() => openPokeBall(ballType)} disabled={!canUse} className={`bg-gradient-to-r ${config.color} hover:opacity-90 disabled:opacity-40 text-white border-0 font-bold shadow-lg transition-all duration-200 hover:shadow-xl`} size="sm">
                      {!levelMet ? `Level ${config.levelRequired}` : !canAfford ? `Need ${config.xpCost - currentXP} XP` : 'Use'}
                    </Button>
                  </div>
        ...
        {lastAcquired && <div className="mt-6 p-6 bg-gradient-to-br from-white/25 to-white/15 dark:from-white/20 dark:to-white/10 rounded-xl text-center border-2 border-white/40 dark:border-white/30 shadow-xl">
            <div className="text-4xl mb-3">
              {lastAcquired.sprite.startsWith('http') ? <img src={lastAcquired.sprite} alt={lastAcquired.name} className="w-20 h-20 mx-auto rounded-xl shadow-lg" /> : <span className="drop-shadow-lg">{lastAcquired.sprite}</span>}
            </div>
